...
btn_filter_container.addEventListener("click", () => {
  if (filter_container.classList.contains("opacity-100")) {
    filter_container.classList.remove("opacity-100", "scale-100", "translate-y-0");
    ...
    filter_container.classList.remove("opacity-0", "scale-95", "-translate-y-2");
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        filter_container.classList.add("opacity-100", "scale-100", "translate-y-0");
      });
    });
  }
});
